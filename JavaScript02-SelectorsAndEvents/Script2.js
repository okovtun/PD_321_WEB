...
function tickCountdown() {
    ...
    let timestamp_in_seconds = timestamp;//Math.floor(timestamp / 1000);
    ...
    document.getElementById("debug_days").innerHTML = `Дней:${timestamp_in_seconds / MILISECONDS_IN_DAY}`;
    document.getElementById("debug_time_of_day").innerHTML = `Время в миллисекундах:${time_of_day}`;
    ...
    let hours = Math.floor(time_of_day / MILISECONDS_IN_HOUR);
    if (hours > 0) time_of_day = (time_of_day % (hours * MILISECONDS_IN_HOUR));

    let minutes = Math.floor(time_of_day / MILISECONDS_IN_MINUTE);
    if (minutes > 0) time_of_day = (time_of_day % (minutes * MILISECONDS_IN_MINUTE));

    let seconds = Math.floor(time_of_day / MILISECONDS_IN_SECOND);
